...
| `chat/{chatId}` | `-` | `0` | `chatId`, `chatType`, `name`, `createdByUserId`, `createdAt`, `lastMessageActivityAt`, `flagCount`, `messagesCount`, `userCount`, `initialMemberUserIds`, `initialMessageId`, `initialMessageText` | `chat/{userId1}/{userId2}` | `-` |
| `chat/{chatId}` | `flag/{userId}` | `0` | `createdAt` | | | | | | | | | | | `flag/{userId}` | `chat` |
| `chat/{chatId}` | `member/{userId}` | `1` | `createdAt`, `messagesUnviewedCount` | | | | | | | | | | | `chat/{chatId}` | `member/{createdAt}` | `member/{userId}` | `chat/{lastMessageActivityAt}` |
...
| `chatMessage/{messageId}` | `-` | `0` | `messageId`, `chatId`, `userId`, `createdAt`, `flagCount`, `isInitial:Boolean`, `lastEditedAt`, `text`, `textTags:[{tag, userId}]` | `chatMessage/{chatId}` | `{createdAt}` |
...
| `user/{userId}` | `banned` | `0` | `userId`, `username`, `bannedAt` | `email/{email}` | `banned` | `phone/{phoneNumber}` | `banned` | `device/{device_id}` | `banned` |
