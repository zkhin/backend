...
def test_get_post_view_strongly_consistent(post_view_dynamo):
    ...
    post_view_dynamo.add_post_view(post_item, user_id, 1, datetime.utcnow())
...
def test_add_views_to_post_view(post_view_dynamo):
    ...
    assert item['lastViewedAt'] == viewed_at.isoformat() + 'Z'
    ...
    assert new_item['lastViewedAt'] == new_viewed_at.isoformat() + 'Z'
...
def test_add_views_to_post_view_does_not_exist(post_view_dynamo):
    with pytest.raises(PostViewDoesNotExist):
        post_view_dynamo.add_views_to_post_view('pid', 'uid', 3, datetime.utcnow())
...
def test_generate_post_views(post_view_dynamo, dynamo_client, posts):
    ...
    post_view_dynamo.add_post_view(post1.item, 'uid1', 1, datetime.utcnow())
    post_view_dynamo.add_post_view(post1.item, 'uid2', 1, datetime.utcnow())
    ...
    post_view_dynamo.add_post_view(post2.item, 'uid1', 1, datetime.utcnow())
...
def test_delete_post_views(post_view_dynamo, dynamo_client, posts):
    ...
    pv_1 = post_view_dynamo.add_post_view(post1.item, 'uid1', 1, datetime.utcnow())
    pv_2 = post_view_dynamo.add_post_view(post1.item, 'uid2', 1, datetime.utcnow())
    pv_3 = post_view_dynamo.add_post_view(post2.item, 'uid1', 1, datetime.utcnow())
