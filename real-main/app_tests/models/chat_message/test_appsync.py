...
def message(chat_message_manager, chat, user1):
    yield chat_message_manager.add_chat_message(chat.id, str(uuid4()), user_id=user1.id)


@pytest.fixture
def system_message(chat_message_manager, chat):
    yield chat_message_manager.add_chat_message(chat.id, str(uuid4()))
...
def test_trigger_notification_blocking_relationship(
    ...
    message1 = chat_message_manager.add_chat_message(chat.id, 'lore', user_id=user1.id)
    message2 = chat_message_manager.add_chat_message(chat.id, 'lore', user_id=user2.id)
...
def test_trigger_notification_system_message(chat_message_appsync, user1, appsync_client, system_message, chat):
    appsync_client.reset_mock()

    chat_message_appsync.trigger_notification('ntype', user1.id, system_message)
    assert len(appsync_client.mock_calls) == 1
    assert len(appsync_client.send.call_args.kwargs) == 0
    assert len(appsync_client.send.call_args.args) == 2
    mutation, variables = appsync_client.send.call_args.args
    ...
    assert variables['input']['messageId'] == system_message.id
    assert variables['input']['chatId'] == chat.id
    assert variables['input']['authorUserId'] is None
    assert variables['input']['authorEncoded'] is None
    assert variables['input']['type'] == 'ntype'
    assert variables['input']['text'] == system_message.item['text']
    assert variables['input']['textTaggedUserIds'] == system_message.item.get('textTaggedUserIds', [])
    assert variables['input']['createdAt'] == system_message.item['createdAt']
    assert variables['input']['lastEditedAt'] is None
