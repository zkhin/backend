...
@pytest.fixture
def group_chat(chat_manager, user1, user2, user3):
    chat_id = str(uuid4())
    chat = chat_manager.add_group_chat(chat_id, user1.id, [user2.id, user3.id])
...
def test_get_author_encoded(chat_message_manager, user1, user2, user3, chat, block_manager):
    # regular message
    message = chat_message_manager.add_chat_message(chat.id, 'lore', user_id=user1.id)
    ...
    message = chat_message_manager.add_chat_message(chat.id, 'lore', user_id=user3.id)
    assert message.get_author_encoded(user1.id) is None

    # test with no author (simulates a system message)
    message = chat_message_manager.add_chat_message(chat.id, 'lore', user_id=user2.id)
...
def test_trigger_notifications_group(chat_message_manager, user1, user2, user3, appsync_client, group_chat):
    message = chat_message_manager.add_chat_message(group_chat.id, 'lore', user_id=user2.id)
    message.appsync = mock.Mock()
    message.trigger_notifications('ntype')
    uid1, uid2 = sorted([user1.id, user3.id])
    assert message.appsync.mock_calls == [
        mock.call.trigger_notification('ntype', uid1, message),
        mock.call.trigger_notification('ntype', uid2, message),
    ]

    # add system message, notifications are triggered automatically
    appsync_client.reset_mock()
    message = chat_message_manager.add_system_message_group_name_edited(group_chat.id, 'new name')
    message.trigger_notifications('ntype')
