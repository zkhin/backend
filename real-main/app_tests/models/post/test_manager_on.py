...
def test_on_post_view_change_update_trending_post_owner(post_manager, post, user, caplog):
    new_focus_item = {'sortKey': f'view/{uuid4()}', 'focusViewCount': 1}
    item_focus_post_owner = {'sortKey': f'view/{post.user_id}', 'focusViewCount': 1}
    item_normal_post_owner = {'sortKey': f'view/{post.user_id}', 'viewCount': 1}

    # trigger for a post that's already been deleted
    post.delete()
    assert post_manager.get_post(post.id) is None
    post_manager.on_post_view_change_update_trending(post.id, new_item=new_focus_item)
    assert post_manager.trending_dynamo.get(post.id) is None

    # trigger for a post that is not COMPLETED
    post_manager.add_post(user, post.id, PostType.IMAGE)
    post = post_manager.get_post(post.id)
    assert post.status is not PostStatus.COMPLETED
    post_manager.on_post_view_change_update_trending(post.id, new_item=new_focus_item)
    assert post_manager.trending_dynamo.get(post.id) is None
    post.delete()

    # trigger for the user's own view of the post (normal and FOCUS view)
    post_manager.add_post(user, post.id, PostType.TEXT_ONLY, text='go go')
    post_manager.on_post_view_change_update_trending(post.id, new_item=item_focus_post_owner)
    ...
    post_manager.on_post_view_change_update_trending(post.id, new_item=item_normal_post_owner)
    trending_item = post_manager.trending_dynamo.get(post.id)
    initial_score = float(trending_item.get('gsiA4SortKey'))
    assert initial_score == pytest.approx(1 + 6 / 7, 0.1)
    assert trending_item.get('gsiA4PartitionKey') == 'post/trending'
...
def test_on_post_view_change_update_trending_non_owner(post_manager, post, user, caplog):
    new_focus_item = {'sortKey': f'view/{uuid4()}', 'focusViewCount': 1}
    new_normal_item = {'sortKey': new_focus_item['sortKey'], 'viewCount': 1}

    trending_item = post_manager.trending_dynamo.get(post.id)
    initial_score = float(trending_item.get('gsiA4SortKey'))
    # trigger for non-owner first view of the post, normal and FOCUS view
    post_manager.on_post_view_change_update_trending(post.id, new_item=new_focus_item)
    trending_item = post_manager.trending_dynamo.get(post.id)
    assert float(trending_item.get('gsiA4SortKey')) == pytest.approx(initial_score * 3, 0.1)
    assert trending_item.get('gsiA4PartitionKey') == 'post/trending'

    post_manager.on_post_view_change_update_trending(post.id, new_item=new_normal_item)
    trending_item = post_manager.trending_dynamo.get(post.id)
    assert float(trending_item.get('gsiA4SortKey')) == pytest.approx(initial_score * 4, 0.1)
    assert trending_item.get('gsiA4PartitionKey') == 'post/trending'

    # trigger for non-owner second view of post, normal and FOCUS views
    post_manager.on_post_view_change_update_trending(post.id, new_item=new_focus_item)
    trending_item = post_manager.trending_dynamo.get(post.id)
    assert float(trending_item.get('gsiA4SortKey')) == pytest.approx(initial_score * 6, 0.1)

    post_manager.on_post_view_change_update_trending(post.id, new_item=new_normal_item)
    trending_item = post_manager.trending_dynamo.get(post.id)
    assert float(trending_item.get('gsiA4SortKey')) == pytest.approx(initial_score * 7, 0.1)

    # trigger for first view of a post by a non-owner (either FOCUS or normal)
    # and post.trending_increment_score returns False
    post_manager.on_post_view_change_update_trending(post.id, new_item={'sortKey': f'view/{uuid4()}'})
    trending_item = post_manager.trending_dynamo.get(post.id)
    assert float(trending_item.get('gsiA4SortKey')) == pytest.approx(initial_score * 7, 0.1)
    assert post.trending_increment_score is False
