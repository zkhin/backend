...
def test_flag_autoarchive(post, user2, user3):
    # test without auto-archiving
    post.should_auto_archive = Mock(return_value=False)
    post.flag(user2.id)
    ...
    post.should_auto_archive = Mock(return_value=True)
    post.flag(user3.id)
...
def test_should_auto_archive_real_and_ian(post):
    # verify generally shouldn't be auto-archived
    assert post.should_auto_archive('rando-uid') is False

    # for the 'real' and 'ian' users, should be auto-archived
    real_uid, ian_uid = 'real-uid', 'ian-uid'
    post.user_manager._real_user_id = real_uid
    post.user_manager._ian_user_id = ian_uid
    assert post.should_auto_archive(real_uid) is True
    assert post.should_auto_archive(ian_uid) is True


def test_should_auto_archive_by_popular_demand(post, user2, user3, user4, user5, user6, user7, view_manager):
    ...
    post.refresh_item()
    assert post.should_auto_archive('rando-uid') is True
