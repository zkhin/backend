...
class UserManager(TrendingManagerMixin, ManagerBase):
    ...
    def create_anonymous_user(self, user_id):
        ...
        try:
            password = self.cognito_client.create_user_pool_entry(user_id, username)
        except (
        ...
        tokens = self.cognito_client.get_user_pool_tokens(user_id, password=password)
        try:
    ...
    def on_forced_deletion_sync_user_status(self, user_id, new_item, old_item=None):
        ...
        if user.is_forced_disabling_criteria_met():
            user.refresh_item()
            if user.item and user.is_forced_disabling_criteria_met():
