...
class Migration:
    """
    Copy images in S3 for each media object from media-object-specific paths
    to post-specific paths. Part of removing support for multiple-image posts.
    """
    ...
    def migrate_media_object(self, item):
        self.copy_s3_objects(item)
        self.dynamo_update_schema_version(item)

    def copy_s3_objects(self, item):
        for size in ALL_SIZES:
            old_path = '/'.join([item['userId'], 'post', item['postId'], 'media', item['mediaId'], size.filename])
            if not self.s3_exists(old_path):
                continue

            new_path = '/'.join([item['userId'], 'post', item['postId'], 'image', size.filename])
            logger.warning(f'MediaObject `{item["mediaId"]}`: s3 copy `{old_path}` -> `{new_path}`')
            self.s3_bucket.Object(new_path).copy({'Bucket': self.s3_bucket.name, 'Key': old_path})
