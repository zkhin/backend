...
| `media/{mediaId}` | `-` | `2` | `postId`, `postedAt`, `userId`, `mediaId`, `mediaStatus`, `mediaType`, `isVerified:Boolean`, `takenInReal:Boolean`, `originalFormat`, `imageFormat`, `width:Number`, `height:Number`, `colors:[{r:Number, g:Number, b:Number}]` | `media/{postId}` | `{mediaStatus}` | `media/{userId}` | `{mediaType}/{mediaStatus}/{postedAt}` |
