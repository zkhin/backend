...
| `media/{mediaId}` | `-` | `2` | `postId`, `postedAt`, `userId`, `mediaId`, `mediaType`, `isVerified:Boolean`, `takenInReal:Boolean`, `originalFormat`, `imageFormat`, `width:Number`, `height:Number`, `colors:[{r:Number, g:Number, b:Number}]` | `media/{postId}` | `-` |
