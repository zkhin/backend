...
const smallGrantData = fs.readFileSync(fixturePath('grant.jpg'))
const smallGrantDataB64 = new Buffer.from(smallGrantData).toString('base64')
const bigGrantData = fs.readFileSync(fixturePath('big-grant.jpg'))
...
test('Add image post verification hidden hides verification state', async () => {
  ...
  await eventually(async () => {
    ...
    expect(data.post.postId).toBe(postId)
    expect(data.post.postStatus).toBe('COMPLETED')
    expect(data.post.isVerified).toBe(true) // even though in reality it failed verification
  })
  ...
  await eventually(async () => {
    const {data} = await ourClient.query({query: queries.post, variables: {postId}})
    expect(data.post.postId).toBe(postId)
    expect(data.post.isVerified).toBe(false)
  })
