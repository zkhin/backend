...
test('Invalid jpeg crops, direct gql data upload', async () => {
  ...
  await expect(
    client.mutate({mutation: mutations.addPost, variables: {postId, imageData, crop}}),
  ...
  resp = await client.mutate({mutation: mutations.addPost, variables: {postId, imageData, crop}})
...
test('Invalid jpeg crops, upload via cloudfront', async () => {
  ...
  let postId = uuidv4()
  let crop = {upperLeft: {x: 1, y: -1}, lowerRight: {x: jpegWidth - 1, y: jpegHeight - 1}}
  await expect(client.mutate({mutation: mutations.addPost, variables: {postId, crop}})).rejects.toThrow(
  ...
  postId = uuidv4()
  crop = {upperLeft: {x: 1, y: 1}, lowerRight: {x: jpegWidth + 1, y: jpegHeight - 1}}
  resp = await client.mutate({mutation: mutations.addPost, variables: {postId, crop}})
  expect(resp.data.addPost.postId).toBe(postId)
...
test('Valid jpeg crop, direct upload via gql', async () => {
  ...
  let resp = await client.mutate({mutation: mutations.addPost, variables: {postId, imageData: jpegData, crop}})
  expect(resp.data.addPost.postId).toBe(postId)
...
test('Valid jpeg crop, upload via cloudfront', async () => {
  ...
  let resp = await client.mutate({mutation: mutations.addPost, variables: {postId, crop}})
  expect(resp.data.addPost.postId).toBe(postId)
  expect(resp.data.addPost.postStatus).toBe('PENDING')
  let uploadUrl = resp.data.addPost.imageUploadUrl
  expect(uploadUrl).toBeTruthy()
