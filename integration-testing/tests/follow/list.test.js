...
test('getFollowe[d|r]Users cant request NOT_FOLLOWING', async () => {
  ...
  let resp = await ourClient.query({query: queries.followedUsers, variables})
  expect(resp['errors'].length).toBeTruthy()
  expect(resp['data']['user']['followedUsers']).toBeNull()
  resp = await ourClient.query({query: queries.followerUsers, variables})
  expect(resp['errors'].length).toBeTruthy()
...
test('getFollowe[d|r]Users queries only allow followStatus FOLLOWING when querying about other users', async () => {
  ...
  resp = await ourClient.query({query: queries.followedUsers, variables: {followStatus: 'REQUESTED', userId}})
  expect(resp['errors'].length).toBeTruthy()
  expect(resp['data']['user']['followedUsers']).toBeNull()
  resp = await ourClient.query({query: queries.followerUsers, variables: {followStatus: 'REQUESTED', userId}})
  expect(resp['errors'].length).toBeTruthy()
  expect(resp['data']['user']['followerUsers']).toBeNull()

  // we can *not* see their DENIED relationships
  resp = await ourClient.query({query: queries.followedUsers, variables: {followStatus: 'DENIED', userId}})
  expect(resp['errors'].length).toBeTruthy()
  expect(resp['data']['user']['followedUsers']).toBeNull()
  resp = await ourClient.query({query: queries.followerUsers, variables: {followStatus: 'DENIED', userId}})
  expect(resp['errors'].length).toBeTruthy()
  expect(resp['data']['user']['followerUsers']).toBeNull()
})
