...
test('Post lifecycle, visibility and trending', async () => {
  ...
  resp = await ourClient.query({query: queries.trendingPostsNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingPostsNew.items).toHaveLength(1)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId1)

  // they check trending posts
  resp = await theirClient.query({query: queries.trendingPostsNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingPostsNew.items).toHaveLength(1)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId1)
  ...
  resp = await ourClient.query({query: queries.trendingPostsNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingPostsNew.items).toHaveLength(2)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId2)
  expect(resp.data.trendingPostsNew.items[1].postId).toBe(postId1)

  // check trending users still empty since the 'free trending point' doesn't apply to users
  resp = await ourClient.query({query: queries.trendingUsersNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingUsersNew.items).toHaveLength(0)
  ...
  resp = await ourClient.query({query: queries.trendingPostsNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingPostsNew.items).toHaveLength(0)

  // check trending users, should be unaffected by post archiving & deleting
  resp = await theirClient.query({query: queries.trendingUsersNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingUsersNew.items).toHaveLength(0)
})
...
test('Non-owner views contribute to trending, filter by viewedStatus, reset & delete clear trending', async () => {
  ...
  resp = await otherClient.query({query: queries.trendingPostsNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingPostsNew.items).toHaveLength(2)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId2)
  expect(resp.data.trendingPostsNew.items[1].postId).toBe(postId1)
  ...
  resp = await ourClient.query({query: queries.trendingPostsNew, variables: {viewedStatus: 'VIEWED'}})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingPostsNew.items).toHaveLength(1)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId1)
  resp = await ourClient.query({query: queries.trendingPostsNew, variables: {viewedStatus: 'NOT_VIEWED'}})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingPostsNew.items).toHaveLength(1)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId2)

  // trending users should be empty
  resp = await otherClient.query({query: queries.trendingUsersNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingUsersNew.items).toHaveLength(0)
  ...
  expect(resp.data.trendingPostsNew.items).toHaveLength(2)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId1)
  expect(resp.data.trendingPostsNew.items[1].postId).toBe(postId2)

  // we should be in trending users
  resp = await otherClient.query({query: queries.trendingUsersNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingUsersNew.items).toHaveLength(1)
  expect(resp.data.trendingUsersNew.items[0].userId).toBe(ourUserId)
  ...
  resp = await otherClient.query({query: queries.trendingUsersNew})
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingUsersNew.items).toHaveLength(2)
  expect(resp.data.trendingUsersNew.items[0].userId).toBe(theirUserId)
  expect(resp.data.trendingUsersNew.items[1].userId).toBe(ourUserId)
...
test('Views of non-original posts contribute to the original post & user in trending', async () => {
  ...
  expect(resp.errors).toBeUndefined()
  expect(resp.data.trendingPostsNew.items).toHaveLength(2)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId3)
  expect(resp.data.trendingPostsNew.items[1].postId).toBe(postId1)
  ...
  expect(resp.data.trendingPostsNew.items).toHaveLength(2)
  expect(resp.data.trendingPostsNew.items[0].postId).toBe(postId1)
  expect(resp.data.trendingPostsNew.items[1].postId).toBe(postId3)
