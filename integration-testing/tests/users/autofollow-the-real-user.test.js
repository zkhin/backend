...
test.skip('new users auto-follow a user with username `real`, if they exist', async () => {
  ...
  let resp = await client.query({query: schema.ourFollowedUsers})
  ...
  await realClient.mutate({mutation: schema.setUsername, variables: {username: 'real'}})
  ...
  resp = await client.query({query: schema.ourFollowedUsers})
  expect(resp['errors']).toBeUndefined()
