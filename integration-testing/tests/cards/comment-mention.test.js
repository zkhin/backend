...
test('CommenttMention card generation and format, fullfilling and dismissing card', async () => {
  ...
  await ourClient.query({query: queries.self}).then(({data: {self: user}}) => {
    ...
    expect(user.cardCount).toBe(2) // one of these is for the comment, another is for the mention
  ...
  await misc.sleep(2000)
  await ourClient.query({query: queries.self}).then(({data: {self: user}}) => expect(user.cardCount).toBe(2))
  ...
  await misc.sleep(2000)
  await otherClient.query({query: queries.self}).then(({data: {self: user}}) => expect(user.cardCount).toBe(0))
