...
test('PostLikes card generation and format', async () => {
  ...
  const cardId = await eventually(async () => {
    ...
    expect(card.action).toMatch(RegExp('^https://real.app/apps/social/user/.*/post/.*/likes'))
  ...
  await eventually(async () => {
    ...
    expect(data.self.cards.items[0].action).toMatch(RegExp('^https://real.app/apps/social/user/.*/post/.*/likes'))
  })
})
