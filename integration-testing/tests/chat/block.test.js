...
test('Blocking a user we are in a group chat with', async () => {
  ...
  expect(resp['data']['chat']['messages']['items'][1]['authorUserId']).toBe(theirUserId)
  expect(resp['data']['chat']['messages']['items'][1]['author']).toBeNull()
...
test('Creating a group chat with users with have a blocking relationship skips them', async () => {
  ...
  resp = await ourClient.mutate({mutation: schema.createGroupChat, variables})
  expect(resp['errors']).toBeUndefined()
  ...
  resp = await theirClient.query({query: schema.chat, variables: {chatId: chatId1}})
  expect(resp['errors']).toBeUndefined()
  expect(resp['data']['chat']).toBeNull()
  ...
  resp = await theirClient.mutate({mutation: schema.createGroupChat, variables})
  expect(resp['errors']).toBeUndefined()
  ...
  resp = await ourClient.query({query: schema.chat, variables: {chatId: chatId2}})
  expect(resp['errors']).toBeUndefined()
  expect(resp['data']['chat']).toBeNull()
})
...
test('Test create a group chat with two users that have a blocking relationship between them', async () => {
  ...
  let resp = await other1Client.mutate({mutation: schema.blockUser, variables: {userId: other2UserId}})
  expect(resp['errors']).toBeUndefined()
  expect(resp['data']['blockUser']['userId']).toBe(other2UserId)
  expect(resp['data']['blockUser']['blockedStatus']).toBe('BLOCKING')
