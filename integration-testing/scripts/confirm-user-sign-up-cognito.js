...
prmt.get(prmtSchema, async (err, result) => {
  ...
  await confirmUser(result.userId, result.confirmationCode, result.pinpointEndpointId, result.pinpointEndpointId)
  if (result.pinpointEndpointId) await trackWithPinpoint(result.userId, result.pinpointEndpointId)
})

const confirmUser = async (userId, confirmationCode, endpointId) => {
  ...
  if (endpointId) opts.AnalyticsMetadata = {AnalyticsEndpointId: endpointId}
  await userPoolClient.confirmSignUp(opts).promise() // empty response upon success
